    ...
    <>
      ...
      <main>
        <section className="card login">
          ...
          <div className="error">
            A aplicação foi publicada, mas as variáveis do Supabase não estão configuradas na Vercel.
          </div>
          <p>Configure estas variáveis em Vercel → Project Settings → Environment Variables:</p>
          <ul>
...
function Login({onLogin}){
  ...
  async function submit(){
    setError('');
    setLoading(true);
    const {data,error}=await supabase.rpc('login_user',{p_code:code,p_password:password});
    setLoading(false);
    if(error){setError(error.message);return}
    if(!data || data.length===0){setError('Código ou senha incorretos.');return}
    onLogin(data[0]);
  }
...
function ChangePassword({user,onDone}){
  ...
  async function submit(){
    setError('');
    if(p1.length<4){setError('A senha deve ter pelo menos 4 caracteres.');return}
    if(p1!==p2){setError('As senhas não coincidem.');return}
    const {error}=await supabase.rpc('change_password',{p_user_id:user.id,p_new_password:p1});
    if(error){setError(error.message);return}
  ...
  return (
    <>
      ...
      <main>
        ...
        <div className="footer">Vincos & Agulha — Aplicação Online V2.0</div>
      </main>
    </>
  );
}
...
function Register({user,clients,load}){
  ...
  async function save(){
    ...
    if(!client){setNotice('Selecione o cliente.');return}
    if(!Number(pieces)||Number(pieces)<1){setNotice('Indique um número válido de peças.');return}
...
function Table({rows}){return <div className="scroll"><table><tbody>{rows.map(([a,b])=><tr key={a}><td>{a}</td><td>{b}</td></tr>)}</tbody></table></div>}
    ...
    <section className="card">
      ...
      <div className="scroll">
        <table>
          ...
          <tbody>{clients.map(c=><tr key={c.id}><td>{c.name}</td><td>{c.phone}</td><td>{c.address}</td><td>{c.active?'Ativo':'Inativo'}</td><td><button className="small-btn secondary" onClick={()=>toggle(c.id)}>{c.active?'Inativar':'Ativar'}</button></td></tr>)}</tbody>
        </table>
      </div>
    </section>
  );
...
function Users({users,manager,load}){
  ...
  async function reset(id){
    ...
    if(!p) return;
    const {error}=await supabase.rpc('reset_user_password',{p_manager_id:manager.id,p_user_id:id,p_temp_password:p});
    if(error) return alert(error.message);
    alert('Senha reposta. O funcionário terá de alterar no próximo acesso.');
    load();
  }
  async function toggle(id){
    const {error}=await supabase.rpc('toggle_user_active',{p_manager_id:manager.id,p_user_id:id});
    if(error) return alert(error.message);
    load();
  }
    ...
    <section className="card">
      ...
      <div className="scroll">
        <table>
          ...
          <tbody>{users.map(u=><tr key={u.id}><td>{u.access_code}</td><td>{u.name}</td><td>{u.role}</td><td>{u.active?'Sim':'Não'}</td><td>{u.must_change_password?'Sim':'Não'}</td><td><button className="small-btn secondary" onClick={()=>reset(u.id)}>Repor senha</button>{u.access_code!=='GER001' && <button className="small-btn secondary" onClick={()=>toggle(u.id)}>{u.active?'Inativar':'Ativar'}</button>}</td></tr>)}</tbody>
        </table>
      </div>
    </section>
  );
    ...
    <section className="card">
      ...
      <div className="scroll">
        <table>
          ...
          <tbody>{records.map(r=><tr key={r.id}><td>{new Date(r.created_at).toLocaleString('pt-PT')}</td><td>{r.client_name}</td><td>{r.user_name}</td><td>{r.user_code}</td><td>{r.pieces}</td><td>{r.notes}</td></tr>)}</tbody>
        </table>
      </div>
    </section>
  );
